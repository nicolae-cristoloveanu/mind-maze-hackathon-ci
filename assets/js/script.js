document.addEventListener("DOMContentLoaded", function () {
    ...
    while (
      ...
      potentialMoves.forEach((move) => {
        if (
          ...
          maze[currentPosition[0]][currentPosition[1]]["walls"][move[2]] ===
            false
        ) {
  ...
  function positionPlayer(direction, openDoor = false) {
    ...
    if (possibleDirections.includes(direction)) {
      ...
      console.log(
        `DEBUG: Current Player position =>${[currentRow, currentCol]}`
      );
  ...
  function askTrivia(gameState) {
    const questionData = gameState.questionBank.retrieveRandomQuestion();
    console.log(`DEBUG: Question=>`, questionData);
    ...
    if (modalFooter) {
      ...
      for (let count = 0; count < questionData.options.length; count++) {
        const dataType =
          questionData.options[count] === questionData.answer
            ? "trivia-correct"
            : "trivia-incorrect";
        ...
        button.innerText = questionData.options[count];
        button.addEventListener("click", triviaEventHandler);
  ...
  function gameStart(gameState) {
    ...
    gameState.reset();
    gameState.questionBank.fetchQuestions(
      gameState.gameLevel.level,
      gameState.gameLevel.numTrivia
    );
  ...
  const questionBank = {
    ...
    async fetchQuestions(difficulty = "medium", numQuestions = 10) {
      try {
        ...
        console.log(
          `DEBUG: Fetched ${newQuestions.length} new questions. Total in bank: ${this.bank.length}`
        );
      } catch (err) {
  ...
  function triviaEventHandler(event) {
    if (event.currentTarget.getAttribute("data-type") === "trivia-correct") {
      gameState.incrementCorrect();
      updateHeadsUpDisplay(gameState);
      checkKeyEvent.detail.status = "open";
